...
LOGIN_FEATURES = env.list(
    "LOGIN_FEATURES",
    default=[],
)

# Reset password link lifetime interval (in seconds). By default: 1 hour.
LOGIN_RESET_PASSWORD_EXPIRATION_DELTA = timedelta(
    seconds=env.int(
        "LOGIN_RESET_PASSWORD_EXPIRATION_DELTA",
        default=3600,
    ),
)

if "LOG_SQL" in LOGIN_FEATURES:  # pragma: no cover
    LOGGING = {
        "version": 1,
